...
$u_name = trim(htmlspecialchars($_GET['u']));


$u_info = mysqli_fetch_array(mysqli_query($_db,""SELECT `username` FROM `users` WHERE `username`='". $u_name ."'")); // взимам отделно името на кеатегорията да мога да го изплюЩяя ейтука в хронологията .. ;d
?>


 <div id='forum-navigation-hron'><a href="./?p=forum">Форум</a> &raquo; Теми от <?php echo $u_info['username'];?></div>
...
<?php
		echo "\n<table cellpadding='15' cellspacing='0' border='0'  class='forum-table' >
				...
				$forum_posts_q = mysqli_query($_db,""SELECT * FROM `forum-posts` WHERE  `username`='".$u_info['username']."' ORDER BY `id` DESC");
					while( $post = mysqli_fetch_array($forum_posts_q))
					{
					  $answers = mysqli_num_rows(mysqli_query($_db,""SELECT `username` FROM `forum-answers` WHERE `post_id`='". $post['id'] . "'")); // Брой отговори
					   ...
					   echo "\n<tr class='row'>\n";
						   echo "
							   <td>".$locked_img ."<a href='./?p=forum&w=read&c=".$post['cat_id']."&read=". $post['id'] . "'>". $post['topic-title'] . "</a></td>
